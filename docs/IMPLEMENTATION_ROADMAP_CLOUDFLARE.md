...
本轮产品反馈修订（2026-08-29）：确认旧版 `?skip=1` / `?fast=1` 入口和 `ui.js` 等待控制器可复用；C_TEST 采用模块内跳过按钮，避免在首页暴露跳过开关。训练目录的完成状态改为右侧灰色勾选标记；移除首页标题副标题；普通账号的锁定提示改为中性文案，不再显示“普通参与者/测试编号”等内部角色信息。首页会话状态区域的容器为 `#participantSessionChip`，文案节点为 `#participantSessionStatus`，集中由 `src/js/main.js` 的 `formatParticipantStatus()` 和各事件处理器维护。
...
- [ ] AI hook 成功或 fallback 后尽力写入 ai_call_events；写入失败异步重试，不阻塞课程。
- [ ] 先选择 2 个代表模块完成端到端记录，再扩展到全部 42 个模块。
- [ ] 保存并接通完整 `conversation_messages` 回放：已完成模块重新打开时只读恢复固定文案、用户回复/选择和 AI 回复，音频允许重新播放。
